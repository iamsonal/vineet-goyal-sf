...
function populate(count, luvio, store) {
    for (let i = 0; i < count; i += 1) {
        ...
        ingestRelatedListInfo(copy, DEFAULT_STORE_ID, luvio, store, TIMESTAMP);
    }
}
...
for (let i = 0; i < WARM_UP_ITERATION_COUNT; i++) {
    ...
    ingestRelatedListInfo(JSON.parse(mockRelatedListInfo), DEFAULT_STORE_ID, luvio, store);
}
    ...
    benchmark('O(n) - ingest 1000 records when store has 10 records', () => {
        ...
        before(() => {
            ...
            for (let i = 0; i < number; i += 1) {
                const copy = clone(mockRelatedListInfo, i);
                next.push(copy);
            }
        });
        ...
        run(() => {
            for (let i = 0; i < number; i += 1) {
                ingestRelatedListInfo(next[i], DEFAULT_STORE_ID, luvio, store, TIMESTAMP);
            }
        });
    });
...
describe('Constant time', () => {
    benchmark('O(1) - ingest 1 record when store has 1000 records', () => {
        ...
        before(() => {
            ...
            next = clone(mockRelatedListInfo);
        });

        run(() => {
            ingestRelatedListInfo(next, DEFAULT_STORE_ID, luvio, store, TIMESTAMP);
        });
    });
