...
function populate(count, luvio, store) {
    for (let i = 0; i < count; i += 1) {
        ...
        ingestRelatedListSummaryInfoCollection(copy, DEFAULT_STORE_ID, luvio, store, TIMESTAMP);
    }
}
...
describe('Constant time', () => {
    benchmark('O(1) - ingest 1 record when store has 1000 records', () => {
        ...
        run(() => {
            ingestRelatedListSummaryInfoCollection(next, DEFAULT_STORE_ID, luvio, store, TIMESTAMP);
        });
    });
