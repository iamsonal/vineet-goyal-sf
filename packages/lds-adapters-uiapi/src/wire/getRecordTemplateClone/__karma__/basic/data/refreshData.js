...
try {
    ...
    const relationshipApiName = 'Account__r';
    await helpers.createCustomField(
        customObject2ApiName,
        lookupFieldApiName,
        'Lookup',
    ...
    const customObject2recordId = await helpers.createTempRecord(customObject2ApiName, {
        ...
        [lookupFieldApiName]: accountRecordId,
    });

    const entries = [
        // Generate mock for Custom_Object__c with no record types
        {
            endpoint: `record-defaults/template/clone/${customObject1recordId}`,
            filename: 'record-template-clone-Custom_Object__c',
        },

        // Generate mocks for CustomObject2__c with 1 record type
        {
        ...
        {
            endpoint: `record-defaults/template/clone/${customObject2recordId}?recordTypeId=${recordTypeId}&optionalFields=${customObject2ApiName}.Owner.Id,${customObject2ApiName}.Owner.Name,${customObject2ApiName}.OwnerId,${customObject2ApiName}.RecordType.Id,${customObject2ApiName}.RecordType.Name,${customObject2ApiName}.RecordTypeId`,
            filename: 'record-template-clone-Custom_Object_2__c-optionalField-RecordType-Owner',
        },
        {
            endpoint: `record-defaults/template/clone/${customObject2recordId}?recordTypeId=${recordTypeId}&optionalFields=${customObject2ApiName}.${lookupFieldApiName},${customObject2ApiName}.${relationshipApiName}.Id,${customObject2ApiName}.${relationshipApiName}.Name`,
            filename: 'record-template-clone-Custom_Object_2__c-optionalField-Lookup',
        },
        {
            endpoint: `record-defaults/template/clone/${customObject2recordId}?recordTypeId=${recordTypeId}&optionalFields=${customObject2ApiName}.${lookupFieldApiName},${customObject2ApiName}.${relationshipApiName}.Id,${customObject2ApiName}.${relationshipApiName}.Name,${customObject2ApiName}.${numberFieldApiName}`,
            filename: 'record-template-clone-Custom_Object_2__c-optionalField-Lookup-Number',
        },
    ];
